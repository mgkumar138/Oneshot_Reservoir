...
def run_a2cagent_multiplepa_expt(b, mtype, env, hp, agent, alldyn, sessions, useweight=None, nocue=None, noreward=None):
    ...
    for t in range(sessions * len(env.rlocs)):
        ...
        while not done:
            ...
            allstate, rfr, rho, value, _, qhat = agent.act(state=state, cue_r_fb=cue)
            ...
            agent.memory.store(state=allstate, action=np.argmax(rho),reward=reward)
...
def setup_a2cagent_multiplepa_expt(hp,b):
    from backend_scripts.model import StochBackpropAgent
    ...
    start = dt.time()
    agent = StochBackpropAgent(hp=hp,env=env)
...
if __name__ == '__main__':
    ...
    hp['exptname'] = '6pa_hid_a2c_{}n_{}ra_{}lr_{}mcb_{}tg_{}dt_b{}_{}'.format(
        hp['nhid'], hp['hidact'], hp['lr'],hp['mcbeta'],
